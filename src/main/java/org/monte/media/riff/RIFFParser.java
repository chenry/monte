/*
 * @(#)RIFFParser.java  1.4  2011-08-26
 *
 ...
 * @version 1.4 2011-08-26 Adds HashMap for stop chunks.
...
public class RIFFParser extends Object {
    ...
    public long parse(InputStream in, RIFFVisitor v)
        ...
        visitor = v;
        parseFile();
        return this.in.getScan();
    }

    public void parse(ImageInputStream in, RIFFVisitor v)
            throws ParseException, AbortException, IOException {
        parse(new ImageInputStreamAdapter(in), v);
    }
    ...
    private void parseFORM(HashMap props)
        ...
        long offset = in.getScan();
        int type = in.readFourCC();

        if (!isGroupType(type)) {
            throw new ParseException("Invalid FORM Type: \"" + type + "\"");
        }
        ...
        try {
            ...
            chunk.setParserMessage(
                    ...
                    + NumberFormat.getInstance().format(in.getScan() - offset)
                    + " bytes");
        } finally {
        ...
        try {
            ...
            while (in.getScan() < finish) {
                ...
                if (id == LIST_ID) {
                    ...
                    parseGarbage(chunk, id, finish-in.getScan(), in.getScan());
                    ParseException pex = new ParseException("Invalid Chunk: \"" + id + "\" at offset:" + idscan);
                    chunk.setParserMessage(pex.getMessage());
                    //throw pex;
                }
