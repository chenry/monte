...
public class QuickTimeWriter extends QuickTimeOutputStream implements MovieWriter {
    ...
    public int addTrack(Format fmt) throws IOException {
        if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
            return addVideoTrack(fmt.get(EncodingKey),
                    fmt.get(CompressorNameKey),
                    fmt.get(FrameRateKey).getNumerator() * fmt.get(FrameRateKey).getDenominator(),
            ...
            boolean signed = fmt.get(SignedKey, sampleSizeInBits > 8);
    ...
    private Codec createCodec(Format fmt) {
        Codec[] codecs = Registry.getInstance().getEncoders(new Format(MimeTypeKey, MIME_QUICKTIME).append(fmt));
    ...
    private void createCodec(Track tr) {
        ...
        if (tr.codec != null) {
            if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
                ...
                tr.codec.setInputFormat(new Format(
                        MimeTypeKey, MIME_JAVA, EncodingKey, ENCODING_BUFFERED_IMAGE,
                        DataClassKey, BufferedImage.class).append(fmt));

                if (null == tr.codec.setOutputFormat(
                        new Format(
                        MimeTypeKey, MIME_QUICKTIME,
                        DataClassKey, byte[].class).append(fmt))) {
    ...
    public void write(int track, Buffer buf) throws IOException {
        ...
        // Encode sample data
        {
            ...
            Buffer outBuf;
            if (tr.format.matches(buf.format)) {
                outBuf = buf;
            } else {
