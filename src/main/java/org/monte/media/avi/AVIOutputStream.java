...
/**
 * Provides low-level support for writing already encoded audio and
 * video samples into an AVI 1.0 file.
 * <p>
 ...
 * @version $Id: AVIOutputStream.java 137 2011-12-28 09:19:37Z werner $
 */
public class AVIOutputStream extends AbstractAVIStream {
    ...
    public int addVideoTrack(String fccHandler, long scale, long rate, int width, int height, int depth, int syncInterval) throws IOException {
        ...
        VideoTrack vt = new VideoTrack(tracks.size(), fccHandler,//
    ...
    /** Adds an audio track.
     *
     * @param waveFormatTag  The format of the audio stream given in MMREG.H,
     * for example 0x0001 for WAVE_FORMAT_PCM.
     * @param scale The numerator of the sample rate.
     * @param rate The denominator of the sample rate.
     ...
     * sampleRate is not equal to the scale, if numberOfChannels is not 1 or 2.
     * @return Returns the track index.
     */
    public int addAudioTrack(int waveFormatTag, //
        ...
        AudioTrack t = new AudioTrack(tracks.size(), "\u0000\u0000\u0000\u0000");
     ...
     * For example, Its your responsibility to only append JPG files if you have
     * chosen the JPEG video format.
     * <p>
     * If you append all frames from files or from input streams, then you
     ...
     * <p>
     * If a track contains palette changes, then all key frames must be immediately
     * preceeded by a palette change chunk. If a key frame is not preceeded by
     * a palette change chunk, it will be downgraded to a delta frame.
     *
    ...
    private void writeProlog() throws IOException {
        ...
        for (Track tr : tracks) {
            ...
            tr.strhChunk = new FixedSizeDataChunk("strh", 56);
            tr.strhChunk.seekToEndOfChunk();
            strlChunk.add(tr.strhChunk);

            tr.strfChunk = new FixedSizeDataChunk("strf", tr.getSTRFChunkSize());
    ...
    private void writeEpilog() throws IOException {
         ...
         */
        {
            ...
            long moviListOffset = moviChunk.offset + 16;

            {
                ...
                for (Sample s : idx1) {
                    ...
                    d.writeInt(typeToInt(s.chunkType)); // dwChunkId
                    ...
                    d.writeInt((s.chunkType.endsWith("pc") ? 0x100 : 0x0)//
        ...
        } AVIMAINHEADER; */
        {
            ...
            d.writeInt(0x10); // flags (0x10 == hasIndex)
        ...
        for (Track tr : tracks) {
            ...
            d.writeInt(typeToInt(tr.fccHandler)); // fccHandler: specifies the codec
