 ...
 * @version $Id: AVIWriter.java 150 2012-01-13 13:08:26Z werner $
 */
public class AVIWriter extends AVIOutputStream implements MovieWriter {
    ...
    private int addVideoTrack(Format vf) throws IOException {
        return addVideoTrack(vf.get(EncodingKey),
    ...
    public void write(int track, BufferedImage image, long duration) throws IOException {
        ...
        // Encode pixel data
        {
            ...
            DataChunk videoFrameChunk = new DataChunk(
                    isKeyframe ? vt.twoCC + "db" : vt.twoCC + "dc");
    ...
    public void write(int track, Buffer buf) throws IOException {
        ...
        // Encode sample data
        {
            if (buf.format.matches(tr.format) && buf.data instanceof byte[]) {
            ...
            if (tr.codec == null) {
                createCodec(tr);
                if (tr.codec == null) {
                    throw new UnsupportedOperationException("No codec for this format " + tr.format + "(" + typeToInt(tr.format.get(EncodingKey)) + ")");
                }
            }
    ...
    private Codec createCodec(Format fmt) {
        ...
        Codec[] codecs = Registry.getInstance().getEncoders(new Format(MimeTypeKey, MIME_AVI).append(fmt));
    ...
    private void createCodec(Track tr) {
        ...
        if (tr.codec != null) {
            if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
                Format vf = fmt;
                tr.codec.setInputFormat(new Format(EncodingKey, ENCODING_BUFFERED_IMAGE, DataClassKey, BufferedImage.class).append(fmt));
                if (null == tr.codec.setOutputFormat(new Format(FixedFrameRateKey, true).append(fmt))) {
