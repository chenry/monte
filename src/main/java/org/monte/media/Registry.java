 ...
 * @version $Id: Registry.java 142 2012-01-12 22:14:09Z werner $
 */
public abstract class Registry {
     ...
     * @return An array of codec class names. If no codec was found, an empty array is returned.
     */
     ...
     * 
     * @param outputFormat The output format.
     * @return An array of codecs. If no codec was found, an empty array is returned.
     */
    ...
    public Codec getCodec(Format inputFormat, Format outputFormat) {
        ...
        for (int i = 0; i < clazz.length; i++) {
            try {
                return ((Codec) Class.forName(clazz[i]).newInstance());
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
     ...
     * Use "Java" for formats which are not tied to a file format. Must not be null.
    ...
    /** Suggests output formats for the given input media format and
        ...
        for (Codec c : codecs) {
            for (Format mf : c.getOutputFormats(null)) {
                if (mf.matches(matchFormat)) {
                    if (inputMediaFormat.matches(mf)) {
